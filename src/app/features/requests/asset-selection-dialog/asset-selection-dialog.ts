...
export class AssetSelectionDialogComponent implements OnInit {
    ...
    totalPages = computed(() => Math.ceil(this.filteredAssets().length / this.pageSize));
    ...
    loadAssets() {
        this.apiService.get<{ items: Asset[] }>('assets?page=0&size=100&unfiltered=true').subscribe({
            next: (response: ApiResponse<{ items: Asset[] }>) => {
                const allAssets = response.data.items || [];
