...
public class DepartmentController {
    ...
    @Inject
    private com.draxlmaier.hub.repository.DepartmentRepository deptRepository;
    ...
    public Response getAllDepartments(
        ...
        PaginatedResponse<DepartmentDTO> result = departmentService.getAll(page, size,  null, null, search);
        return Response.ok(
            new ApiResponse<>(
    ...
    public Response deleteDepartment(@PathParam("id") Long id) {
        ...
        if (currentUser == null || currentUser.getRole() == null ||
            !RoleEnum.isEqual(currentUser.getRole().getName(), RoleEnum.ADMIN)) {
            return Response.status(Response.Status.FORBIDDEN)
                .entity(new ApiResponse<>("Only admins can delete departments", null))
                .build();
        }

        long empCount = deptRepository.countEmployeesInDepartment(id);
        if (empCount > 0) {
            return Response.status(Response.Status.CONFLICT)
                .entity(new ApiResponse<>("Cannot delete department: " + empCount + " employee(s) still assigned", null))
                .build();
        }
