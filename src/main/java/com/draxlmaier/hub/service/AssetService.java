...
public class AssetService implements IService<Asset, AssetDTO> {
    ...
    @Override
    public PaginatedResponse<AssetDTO> getAll(int page, int size, String status, String category, String search) {
        return getAll(page, size, status, category, search, null);
    }

    public PaginatedResponse<AssetDTO> getAll(int page, int size, String status, String category, String search, Long employeeId) {
        var items = assetRepository.findAllPaginated(page, size, status, category, search, employeeId)
            .stream()
            .map(assetMapper::toDTO)
            .toList();

        long total = assetRepository.countWithFilters(status, category, search, employeeId);

        return new PaginatedResponse<>(items, total, page, size);
    }

    public PaginatedResponse<AssetDTO> getAllByEmployee(int page, int size, String status, String category, String search, Long employeeId) {
        return getAll(page, size, status, category, search, employeeId);
    }
