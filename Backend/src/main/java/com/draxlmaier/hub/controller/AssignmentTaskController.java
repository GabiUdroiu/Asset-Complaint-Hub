...
public class AssignmentTaskController {
        ...
        try {
            if (!isAuthorizedToAssignTask()) {
                return Response.status(Response.Status.FORBIDDEN)
                        .entity("You are not authorized to assign tasks").build();
            }
    ...
    public Response update(@PathParam("id") Long id, AssignmentTaskDTO dto) {
        if (!isAuthorizedToAssignTask()) {
            return Response.status(Response.Status.FORBIDDEN)
                    .entity("You are not authorized to modify assignment tasks").build();
        }
    ...
    private boolean isAuthorizedToAssignTask() {
        ...
        try {
            ...
            return "DEPT_RESPONSIBLE".equals(role) || "ADMIN".equals(role);
        } catch (Exception e) {
            return false;
        }
    }
